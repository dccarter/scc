...
#define _NODE_CTOR(Tp)                           \
    ...
    void Invoke::toString(Formatter& fmt) const
    {
        ...
        fmt(true) << " " << Lib.Content << "::" << Generator.Content << "." << Function.Content;
        fmt(true) << '(';
        Params.toString(fmt);
        fmt(true) << ')';
    }
    ...
    {
        fmt() << "#ifndef IOD_SYMBOL_" << Name;
        fmt() << "#define IOD_SYMBOL_" << Name;
        fmt.push(true) << "iod_define_symbol(" << Name << ")";
        fmt.pop(true) << "#endif";
    }
