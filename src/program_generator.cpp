...
namespace scc {
    ...
    void ProgramGenerator::generateHeader(const Program &pg, const std::filesystem::path &output)
    {
        ...
        fmt() << "#pragma once";
        fmt() << "//";
        fmt() << "// !!!Generated by scc DO NOT MODIFY!!!";
        fmt() << "// file: " << output;
        fmt() << "// date: " << __DATE__ << " " << __TIME__;
        fmt() << "//";
        fmt();
        ...
        if (pg.space) {
            ...
            fmt();
            fmt() << "#pragma GCC diagnostic push";
            fmt() << R"(#pragma GCC diagnostic ignored "-Wattributes")";
            fmt();
            fmt() << "namespace ";
            pg.space.Name.toString(fmt);
            fmt(true) << " {";
            fmt.push(false);
        ...
        for (const auto& gen: tp.Generators) {
            ...
            for (const auto& p: gen.Params) {
                ...
                if (auto hg = findSourceGenerator(lib, name).lock()) {
                    fmt() << "#pragma region " << lib << "/" << name;
                    hg->generate(!fmt, tp);
                    fmt() << "#pragma endregion " << lib << "/" << name;
                    fmt();
                }
            }
        }
